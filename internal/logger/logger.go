...
func InitLogger(cfg *base.Config) (err error) {
...
func WriteLogWithCtx(ctx *gin.Context, logType string, msg string, values ...string) error {
	return WriteLog(logType, msg, append([]string{"requestId", ctx.GetString("requestId")}, values...)...)
}

func WriteLog(logType string, msg string, values ...string) error {
	if len(values)%2 != 0 {
		return fmt.Errorf("values must be paired")
	}
